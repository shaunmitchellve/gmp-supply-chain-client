...
import {getArrivalTime, fetcher} from '@/app/lib/utils';
      ...
      <div className="h-full">
        <APIProvider apiKey={apiKey.key}>
          <Map
            ...
            className={clsx({
              'h-[calc(100vh-65px)] md:h-[calc(100vh-100px)] shadow-lg z-10 shadow-gray-400/50':
                true,
            })}
          >
            <StartDriving
              ...
              updateDestinationProps={setDestinationProps}
            />
          </Map>
        </APIProvider>
        <Destination
          arrivalTime={destinationProps.arrivalTime}
          time={destinationProps.time}
          distance={destinationProps.distance}
        />
      </div>
    </>
  );
...
function StartDriving({
  ...
  if (index >= 1) {
    ...
    map.setHeading(
      geometryLibrary.spherical.computeHeading(polyLine[0], polyLine[1])
    );
